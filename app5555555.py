...
# Create Flask app FIRST (before any initialization)
app = Flask(__name__)
...
# Health check route (MUST respond immediately for Render)
...
def query_endpoint():
    ...
    try:
        ...
        result = ATTRIBUTION_ENGINE.query_archive(
            ...
            include_raw_chunks=data.get('include_raw_chunks', True)
        )
...
if __name__ == "__main__":
    ...
    logger.info("✓ Flask server starting (health endpoint ready)")
    logger.info("=" * 70)
    
    # Start Flask app immediately (don't wait for initialization)
